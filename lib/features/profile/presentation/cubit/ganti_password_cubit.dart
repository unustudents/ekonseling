...
class GantiPasswordCubit extends Cubit<GantiPasswordState> {
  ...
  Future<void> onUpdatePassword(
    ...
    try {
      final PostgrestMap? passHash = await SupabaseConfig.client
          .from('users')
          .select('pass_hash')
      ...
      if (passHash == null || passHash.isEmpty) {
        emit(state.copyWith(
            status: GantiPasswordStatus.failure,
            msg: "Error ketika memeriksa password"));
        return;
      }
      if (passHash.isNotEmpty) {
        ...
        if (response) {
          await SupabaseConfig.client.auth
              .updateUser(UserAttributes(password: hashNew));
          await SupabaseConfig.client
              .from('users')
              .update({'pass_hash': hashNew}).eq(
                  'id', SupabaseConfig.client.auth.currentUser!.id);
          emit(state.copyWith(
              status: GantiPasswordStatus.success,
      ...
      emit(state.copyWith(
          status: GantiPasswordStatus.failure,
          msg: "Error ketika mengubah password"));
    }
  }
